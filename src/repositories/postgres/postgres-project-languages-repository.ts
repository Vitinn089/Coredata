...
export class PostgresProjectLanguagesRepository implements ProjectLanguagesRepository {
	...
	async get(project_id?: string) {
		this.#sql = !project_id 
			? 'SELECT project_id, language_id FROM tb_project_languages;' 
			: 'SELECT project_id, language_id FROM tb_project_languages WHERE project_id=$1;';
	...
	async getLanguages(project_id: string) {
		this.#sql = 'SELECT project_id, language_name FROM tb_project_languages RIGHT JOIN tb_languages ON tb_project_languages.language_id = tb_languages.language_id  WHERE project_id=$1;';
